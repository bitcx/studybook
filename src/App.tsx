...
const App: React.FC = () => {
  ...
  const navigateToNode = useCallback((path: string, node: PageNode) => {
  ...
  useEffect(() => {
    ...
    setTree(built);
    const first = findFirstPage(built);
    if (first) {
      const node = findNode(built, first);
      if (node) navigateToNode(first, node);
    } else {
      setLoading(false);
    }
  }, [navigateToNode]);
  ...
  const handleNavigate = useCallback((path: string) => {
    ...
    if (node) navigateToNode(path, node);
  }, [tree, navigateToNode]);
